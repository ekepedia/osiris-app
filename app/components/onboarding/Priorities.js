...
class Priorities extends React.Component {
    ...
    syncWithAPI() {
        ...
        AuthService.getCurrentUserId().then((user_id) => {
            UserPreferenceService.getUserPreference({client, user_id, type_id: COMMON.CONSTS.PREFERENCE_TYPES.JOB_PRIORITIES}).then((user_preferences) => {
    ...
    addUserPreference({preference_id}) {
        ...
        AuthService.getCurrentUserId().then((user_id) => {
            UserPreferenceService.createUserPreference({client, user_id, preference_id, type_id: COMMON.CONSTS.PREFERENCE_TYPES.JOB_PRIORITIES}).then((user_preference_id) => {
    ...
    render() {
        ...
        return (<div className={classes.container}>
            <div className={mc(classes.headerIcon)}>
                <img className={mc(classes.headerIconImg)} src={"/img/priorities-icon.png"}/>
            </div>
            <div className={mc(classes.headerTitle)}>Job Priorities</div>
            <div className={mc(classes.headerSubTitle)}>As you look for a job, what three things matter to you most?</div>

            <div>
                <div style={{display: "flex"}}>
                    <div style={{flex: 1, paddingRight: "5px"}}>
                        {COMMON.CONSTS.JOB_PRIORITIES.map((job_priority) => {

                            let found_check = false;
                            user_preferences.forEach((user_preference) => {
                                if ((user_preference.preference_id + "") === (job_priority.value + "") ) {
                                    found_check = true
                                }
                            });

                            return (<div key={job_priority.label} style={{marginBottom: "8px"}}>
                                <StandardCheckbox label={job_priority.label} value={found_check} padded={true} update={(checked) => {
                                    ...
                                    if (!found && checked) {
                                        console.log("ADD!");
                                        this.addUserPreference({preference_id: job_priority.value })
...
export default withApollo(withRouter(injectSheet(Styles)(Priorities)));

...
class Industries extends React.Component {
    ...
    syncWithAPI() {
        ...
        AuthService.getCurrentUserId().then((user_id) => {
            UserPreferenceService.getUserPreference({client, user_id, type_id: COMMON.CONSTS.PREFERENCE_TYPES.INDUSTRIES}).then((user_preferences) => {
    ...
    addUserPreference({preference_id}) {
        ...
        AuthService.getCurrentUserId().then((user_id) => {
            UserPreferenceService.createUserPreference({client, user_id, preference_id, type_id: COMMON.CONSTS.PREFERENCE_TYPES.INDUSTRIES}).then((user_preference_id) => {
    ...
    render() {
        ...
        return (<div className={classes.container}>
            <div className={mc(classes.headerIcon)}>
                <img className={mc(classes.headerIconImg)} src={"/img/industries-icon.png"}/>
            </div>
            <div className={mc(classes.headerTitle)}>Industries of Interest</div>
            <div className={mc(classes.headerSubTitle)}>Which industries interest you the most? Search for an industry</div>
                    ...
                    <div style={{flex: 1, paddingRight: "5px"}}>
                        <StandardMultiSelect
                            ...
                            value={(user_preferences || []).map((user_preference) => ({
                                ...
                                label: (COMMON.CONSTS.INDUSTRIES_MAP[user_preference.preference_id] || {}).label
                            }))}
                            options={COMMON.CONSTS.INDUSTRIES}
                            onChange={(ids) => {
                                console.log("ids", ids);
...
export default withApollo(withRouter(injectSheet(Styles)(Industries)));
